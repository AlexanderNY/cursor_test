...
export const gameService = {
  ...
  async getBotDiagnostics(botId: number): Promise<GameBotDiagnostics> {
    try {
      const response = await apiClient.get<GameBotDiagnostics>(
        `/tg/game/admin/bots/${botId}/diagnostics`,
      )
      return response.data
    } catch (error) {
      throw new Error(getErrorMessage(error))
