...
export function CompetitorsPage() {
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Competitors"
        description="Что вышло за сутки, сравнение с вашим брендом и алерты о новых постах"
      />
          ...
          <CardContent className="space-y-3">
            {!selected && (
              <p className="text-sm text-[var(--text-muted)]">
                Выберите конкурента слева, чтобы увидеть посты и дайджест.
              </p>
            )}
            {selected && (
              <>
                ...
                {compare && (
                  <div className="rounded border border-[var(--border-color)] p-3 text-sm space-y-1">
                    <p>
                      Own: {compare.own.posts_count} posts · avg {compare.own.avg_length} chars
                    </p>
                    <p>
                      Competitor: {compare.competitor.posts_count} posts · avg{' '}
