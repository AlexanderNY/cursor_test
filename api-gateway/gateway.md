...
CORS
    CORS настраивается только на api-gateway (внутренние сервисы CORS не используют).
    allow_origins — явный список доменов UI через env CORS_ORIGINS (через запятую), без "*".
    Новый домен UI → добавить в CORS_ORIGINS в docker-compose / K8s.
