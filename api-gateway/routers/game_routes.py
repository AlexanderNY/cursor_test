...
@router.get("/tg/game/admin/bots/{bot_id}/diagnostics")
async def get_game_bot_diagnostics(
    bot_id: int,
    request: Request,
    current_user: dict | None = Depends(get_current_user),
) -> Response:
    _require_admin(current_user)
    return await _forward_game_admin(f"/tg/game/admin/bots/{bot_id}/diagnostics", request)
