...
def _build_catalog_text(node_views: list[GameMenuNodeView]) -> str:
    lines: list[str] = []
    for idx, nv in enumerate(node_views, start=1):
        node = nv.node
        lines.append(f"{idx}. {node.title}")
        if node.body_text:
            lines.append(f"   {node.body_text.strip()}")
        if nv.has_children:
            lines.append("   (раздел)")
        else:
            lines.append(f"   {_format_price(node.price)}")
        lines.append("")
    return "\n".join(lines).strip()
...
async def _send_menu_level(
    ...
    *,
    ...
    if header_override:
        ...
        parent_node = await menu_repository.get_node(parent_id)
        header = parent_node.body_text or parent_node.title if parent_node else mode.title
    ...
    if not node_views:
        ...
        await bot.send_message(
            chat_id,
            f"{header}\n\nВ этом разделе пока нет пунктов."[:4096],
            reply_markup=empty_kb,
        )
        return

    catalog = _build_catalog_text(node_views)
    caption = f"{header}\n\n{catalog}"[:4096]
    ...
    *,
    ...
    try:
        if node.image_file_id:
            await bot.send_photo(chat_id, photo=node.image_file_id, caption=text, reply_markup=kb)
            return
        photo_file = await _load_node_photo(node)
        if photo_file:
            sent = await bot.send_photo(chat_id, photo=photo_file, caption=text, reply_markup=kb)
            if sent.photo:
                await menu_repository.set_node_image_file_id(node.id, sent.photo[-1].file_id)
            return
        await bot.send_message(chat_id, text, reply_markup=kb)
    except TelegramBadRequest as exc:
        logger.warning("Failed to send menu leaf node_id=%s: %s", node.id, exc)
        await bot.send_message(chat_id, text, reply_markup=kb)
