...
from schemas_game import GameBotCreate, GameBotOut, GameBotUpdate, GameBotDiagnosticsOut
from services.bot_diagnostics import run_bot_diagnostics
...
@router.get("/{bot_id}/diagnostics", response_model=GameBotDiagnosticsOut, dependencies=[Depends(verify_game_admin_token)])
async def get_bot_diagnostics(bot_id: int) -> GameBotDiagnosticsOut:
    row = await game_repository.get_bot(bot_id)
    if not row:
        raise HTTPException(status_code=404, detail="Bot not found")
    return await run_bot_diagnostics(bot_id)
