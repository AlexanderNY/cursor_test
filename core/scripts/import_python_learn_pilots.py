"""Merge Learn Python series .md articles into learn_seed.json."""
...
SERIES_DIR = ROOT / "deploy" / "ui-9to18" / "src" / "data" / "learn" / "python"
...
def main() -> None:
    ...
    files = sorted(SERIES_DIR.glob("py*.md"))
    if not files:
        raise SystemExit(f"no py*.md in {SERIES_DIR}")
    for path in files:
        ...
        if quiz_count < 3 or anki_count < 3:
            raise SystemExit(f"{slug}: need >=3 quiz and anki, got {quiz_count}/{anki_count}")
        if slug in by_slug:
            ...
            print(f"appended {slug} order={payload.get('order')} quiz={quiz_count} anki={anki_count}")
