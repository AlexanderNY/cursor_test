"""Merge Learn SA (system analyst) series .md articles into learn_seed.json."""
...
SERIES_DIR = ROOT / "deploy" / "ui-9to18" / "src" / "data" / "learn" / "sa"
...
def main() -> None:
    ...
    files = sorted(SERIES_DIR.glob("sa-*.md"))
    if not files:
        raise SystemExit(f"no sa-*.md in {SERIES_DIR}")
    for path in files:
        ...
        if "analyst" not in profiles:
            raise SystemExit(f"{slug}: profiles must include analyst, got {profiles}")
        if slug in by_slug:
